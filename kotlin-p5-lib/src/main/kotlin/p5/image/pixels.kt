...
inline val p5.pixels: Array<Byte> get() = asDynamic().pixels as Array<Byte>
 ...
 *
 * THRESHOLD
 * Converts the image to black and white pixels depending on if they are above or
 ...
 * parameter. The parameter can be set to value between 2 and 255, but
 ...
 */
inline fun p5.filter(filterType: String, filterParam: dynamic = undefined) {
 ...
 *
 * If you want to extract an array of colors or a sub image from a p5.Image object,
 ...
 */
inline fun p5.get(x: Int, y: Int, w: Int, h: Int): p5.Image = asDynamic().get(x ,y, w, h) as p5.Image

inline fun p5.get(): p5.Image = asDynamic().get() as p5.Image

inline fun p5.get(x: Int, y: Int): Array<Int> = asDynamic().get(x ,y) as Array<Int>
 ...
 */
inline fun p5.loadPixels() { asDynamic().loadPixels() }
 ...
 * @param {Number} [x] x-coordinate of the pixel
 * @param {Number} [y] y-coordinate of the pixel
 * @param {Number|Number[]|Object} [imgOrCol] insert a grayscale value | a pixel array |
 ...
 * square with orange-brown gradient lightening at bottom right.
 * image of the rocky mountains. with lines like an 'x' through the center.
 */
inline fun p5.set(x: Int, y: Int, imgOrCol: Int) { asDynamic().set(x, y, imgOrCol) }

inline fun p5.set(x: Int, y: Int, imgOrCol: Array<Int>) { asDynamic().set(x, y, imgOrCol) }

inline fun p5.set(x: Int, y: Int, imgOrCol: p5.Image) { asDynamic().set(x, y, imgOrCol) }

inline fun p5.set(x: Int, y: Int, imgOrCol: p5.Color) { asDynamic().set(x, y, imgOrCol) }
 ...
 * two images of the rocky mountains. one on top, one on bottom of canvas.
 */
inline fun p5.updatePixels(x: Int, y: Int, w: Int, h: Int) {
    asDynamic().updatePixels(x, y, w, h)
}

inline fun p5.updatePixels() {
