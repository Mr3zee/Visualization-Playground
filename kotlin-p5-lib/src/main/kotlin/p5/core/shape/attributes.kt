/**
 * @file Attributes are an important part of the general API for the web,
 * but the attributes.js module defines and handles miscellaneous graphical attributes
 * that can be considered as a set of active parameters for the current drawing style.
 * Think of this as a grab bag of all the remaining drawing parameters that were not
 * moved out into separate dedicated modules as with color and typography.
 */
 ...
 * <div>
 * <code>
 * // Example showing RADIUS and CENTER ellipse mode with 2 overlaying ellipses
 * ellipseMode(RADIUS);
 * fill(255);
 ...
 */
inline fun p5.ellipseMode(m: String): p5 = asDynamic().ellipseMode(m) as p5
 ...
 * smooth (anti aliased) edges on your geometry.
 *
 * @method noSmooth
 * @chainable
 * @example
 ...
 */
inline fun p5.noSmooth(): p5 = asDynamic().noSmooth() as p5
 ...
 */
inline fun p5.rectMode(m: String): p5 = asDynamic().rectMode(m) as p5
 ...
 */
inline fun p5.smooth(): p5 = asDynamic().smooth() as p5
 ...
 */
inline fun p5.strokeCap(cap: String): p5 = asDynamic().strokeCap(cap) as p5
 ...
 */
inline fun p5.strokeJoin(join: String): p5 = asDynamic().strokeJoin(join) as p5
 ...
 * strokeWeight(1); // Resulting stroke weight is 5
 ...
 * 2 horizontal black line. Top line: thin, bottom line: 5 times thicker than top
 */
inline fun p5.strokeWeight(w: Double? = null): p5 = asDynamic().strokeWeight(w) as p5