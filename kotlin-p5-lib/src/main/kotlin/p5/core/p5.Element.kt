/**
 * @file Although p5.js is a general toolkit for all sorts of interactive art projects,
 * in practice many p5.js sketches focus on rendering two-dimensional graphics into a <canvas> element.
 * The p5.Element.js module is a wrapper around the browser's DOM API which is focused
 * on the <canvas> element and also handles user input events like clicks and mouse movements.
 * Conceptually, p5.js considers the sketch to be the entire web page.
 * It's useful to separate <canvas> from the more general DOM because so many p5.js
 * projects focus on <canvas> and this enables smaller custom builds.
 */
